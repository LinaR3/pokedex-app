...
/* ════════════════════════════════════════
   FETCH DETAIL — FIX: una sola llamada a species
   (antes hacía fetch dos veces a la misma URL)
════════════════════════════════════════ */
export async function fetchDetail(categoryId, id) {
  ...
  if (categoryId === 'pokemon' && data.species?.url) {
    try {
      // UNA sola llamada a species — se reutiliza para obtener evolution_chain
      const speciesData = await fetchWithRetry(data.species.url).then(r => r.json())

      const evolutionData = speciesData.evolution_chain?.url
        ? await fetchWithRetry(speciesData.evolution_chain.url).then(r => r.json())
        : null
      ...
      console.warn('⚠️ Species/evolution fetch failed:', e)
    }
  }
